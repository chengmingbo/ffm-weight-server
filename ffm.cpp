...
string& ffm_weights(
	...
	cJSON_AddItemToObject(js, "vivj_info", js_array);
	cJSON_AddNumberToObject(js, "size", end-begin);
    ...
    for(ffm_node *N1 = begin; N1 != end; N1++)
    {
        ...
        for(ffm_node *N2 = N1+1; N2 != end; N2++)
        {
			...
			cJSON_AddNumberToObject(js_item, "i_idx", N1->j);
			cJSON_AddNumberToObject(js_item, "i_field", N1->f);
			cJSON_AddNumberToObject(js_item, "i_val", N1->v);
			cJSON_AddNumberToObject(js_item, "j_idx", N2->j);
			cJSON_AddNumberToObject(js_item, "j_field", N2->f);
			cJSON_AddNumberToObject(js_item, "j_val", N2->v);
			cJSON_AddNumberToObject(js_item, "weight", e);
			cJSON_AddItemToArray(js_array, js_item);
        }
    }
	...
	cJSON_AddNumberToObject(js, "wtx", t);
	cJSON_AddNumberToObject(js, "ctr", 1/(1+exp(-t)));

	char* js_out = cJSON_Print(js);
	out = string(js_out);
...
string& ffm_weights(
    ...
    for(ffm_node *N1 = begin; N1 != end; N1++) {
        ...
        for(ffm_node *N2 = N1+1; N2 != end; N2++) {
			...
			cJSON_AddNumberToObject(js_item, "j_field", N2->f);
			cJSON_AddNumberToObject(js_item, "j_val", N2->v);
			cJSON_AddNumberToObject(js_item, "weight", each_w);
			cJSON_AddItemToArray(js_array, js_item);
			//printf("%.2f\n", each_w);
        }
		//cJSON_Delete(js_weight_info);
    }
	cJSON_AddNumberToObject(js, "wtx", t);
	cJSON_AddNumberToObject(js, "ctr", 1/(1+exp(-t)));
